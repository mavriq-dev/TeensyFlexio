...
//#define HARDWARE_CS
#ifdef HARDWARE_CS
TeensyFlexSPI SPIFLEX(2, 3, 4, 5); // Setup on (int mosiPin, int sckPin, int misoPin, int csPin=-1) :
#define assert_cs()
#define release_cs()
#else
TeensyFlexSPI SPIFLEX(2, 3, 4, -1); // Setup on (int mosiPin, int sckPin, int misoPin, int csPin=-1) :
...
void loop() {
  SPIFLEX.beginTransaction(TeensyFlexSPISettings(20000000, MSBFIRST, SPI_MODE0));
  assert_cs();
