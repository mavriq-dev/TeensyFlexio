...
TeensyFlexSerial TeensySerials[] = {
  {2, -1, 1, 0, 0},
  {3, -1, 1, 1, 1},
  {4, -1, 1, 2, 2},
  {5, -1, 1, 3, 3},
...
const uint8_t count_flexSerials = sizeof(TeensySerials) / sizeof(TeensySerials[0]);
...
void setup() {
  ...
  for (uint8_t i = 0; i < count_flexSerials; i++) {
    Serial.printf("\nFlexSerial object begin #:%d, count of serials:%d\n", i, count_flexSerials);
    TeensySerials[i].begin(BAUD);
    // if (!TeensySerials[i].) Serial.println("   *** Failed ***\n");
    streams[7+i] = &TeensySerials[i];
    outputStr(&TeensySerials[i],"TeensySerial", i);
  }
  delay(500);
